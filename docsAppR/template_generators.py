...
class InfoTemplateGenerator(BaseTemplateGenerator):
    ...
    # Labels MUST match the keys in build_field_mapping() in tasks.py exactly
    # (case-sensitive). The population step (populate_excel_templates) clears and
    # re-fills Column C using those keys, so any mismatch causes data to be wiped.
    FIELD_MAPPING = [
    ...
    def generate(self):
        ...
        for row_num, (label, field_name) in enumerate(self.FIELD_MAPPING, start=1):
            ...
            # Format value — date formats must match build_field_mapping() in tasks.py
            if value is None:
                value = ''
