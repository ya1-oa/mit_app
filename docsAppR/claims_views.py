...
def save_rooms(request):
    ...
    try:
        ...
        def _create_rooms_for_client(target_client, room_list):
            """Create Room + RoomWorkTypeValue records for a client."""
        ...
        with transaction.atomic():
            # Save normal rooms to the primary claim
            _create_rooms_for_client(client, normal_rooms)

            # Create MIT sub-claim if MIT rooms were selected
            if mit_rooms:
                mit_client_obj = _clone_client(client, 'MIT')
                _create_rooms_for_client(mit_client_obj, mit_rooms)
                sub_claims_created.append(f"{mit_client_obj.pOwner} ({len(mit_rooms)} rooms)")
            else:
                request.session.pop('mit_claim_id', None)

            # Create RHT sub-claim if RHT rooms were selected
            if rht_rooms:
                rht_client_obj = _clone_client(client, 'RHT')
                _create_rooms_for_client(rht_client_obj, rht_rooms)
                sub_claims_created.append(f"{rht_client_obj.pOwner} ({len(rht_rooms)} rooms)")
            else:
...
def create_claim_step3(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # 'readings_8000' and 'readings_9000' belong exclusively to the MIT/RHT
            # sub-claims created in Step 2.  The primary claim only receives the base
            # list ('basic') and optional 70000s stabilisation ('readings default').
            # This prevents 8000s/9000s rooms from appearing inside the primary claim.
            ...
            primary_templates    = [t for t in encircle_templates if t not in _SUB_TEMPLATES]

            # If 700 (HMR) is selected it gets its own claim — exclude it from
            # the primary so the 700-series entries only appear in the HMR claim.
            _wt_ints_check = [int(w) for w in selected_work_types if str(w).strip().isdigit()]
            primary_work_types = (
                [w for w in selected_work_types if str(w).strip().isdigit() and int(w) != 700]
                if 700 in _wt_ints_check else selected_work_types
            )
