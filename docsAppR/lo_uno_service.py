...
def _is_valid_ooxml(filepath):
    """
    Returns True if the file is a valid OOXML container (xlsx/xlsm).
    Returns False if it is an ODS file masquerading as .xlsx — which happens
    when LibreOffice saves without an explicit FilterName and defaults to ODF.
    """
    import zipfile
    try:
        with zipfile.ZipFile(filepath, 'r') as z:
            names = z.namelist()
            # OOXML containers have [Content_Types].xml; ODS containers have mimetype
            if '[Content_Types].xml' in names:
                return True
            if 'mimetype' in names:
                return False  # ODS/ODF container
            return False  # Neither — unrecognised format
    except Exception:
        return False  # Not a ZIP at all — definitely corrupt
...
def _open_document(desktop, filepath, uno):
    ...
    # Do NOT specify FilterName here — forcing it breaks xlsm files that
    # LO would otherwise open fine via auto-detection.  ODS prevention is
    # handled at save time by always calling storeToURL with an explicit filter.
...
def _save_and_close(doc):
    """
    Save the document and close it, always using an explicit OOXML FilterName.

    We never call doc.store() because in headless mode LibreOffice silently
    picks ODS as its preferred format when it detects any feature that doesn't
    round-trip perfectly through XLSX.  storeToURL with an explicit FilterName
    forces XLSX/XLSM output regardless of LO's internal preference.
    """
    from com.sun.star.beans import PropertyValue
    url = doc.getURL()
    filter_name = (
        'Calc MS Excel 2007 VBA XML' if url.lower().endswith('.xlsm')
    ...
    try:
        ...
        logger.error(f"storeToURL({filter_name}) failed ({e})")
        raise
    finally:
        try:
            doc.close(True)
...
def populate_jobinfo_batch(filepaths_and_mappings):
    ...
    for filepath, field_mapping in filepaths_and_mappings:
        ...
        doc = None
        try:
            # Reject ODS files before handing to LO — they were produced by a
            # previous save that omitted FilterName.  Caller is responsible for
            # re-copying the source template before retrying.
            if not _is_valid_ooxml(filepath):
                logger.error(
                    f"UNO batch: {filename} is ODS/corrupt (not OOXML) — "
                    "re-copy source template before regenerating"
                )
                results[filepath] = -1
                continue
