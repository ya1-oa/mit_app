...
class EmailForm(forms.Form):
    ...
    to = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            ...
            'class': 'form-control recipient-field',
            'autocomplete': 'off',
        }),
        help_text='Separate multiple addresses with commas or semicolons',
    )
    cc = forms.CharField(
        label='CC',
        required=False,
    ...
    claim = forms.ModelChoiceField(
        ...
        label='Link to Claim',
        widget=forms.Select(attrs={'id': 'id_claim', 'class': 'form-control'}),
    )
    # Populated by JS when user checks contacts from the claim panel
    claim_contacts = forms.CharField(required=False, widget=forms.HiddenInput())
    ...
    # ── Attachments: existing Documents ──────────────────────────────────────
    ...
    # ── Attachments: app-generated files ─────────────────────────────────────
    generated_files = forms.ModelMultipleChoiceField(
        queryset=GeneratedFile.objects.order_by('-created_at'),
        widget=forms.CheckboxSelectMultiple,
        required=False,
        label='App-Generated Files',
        help_text='PDFs, Excel reports, and invoices generated by the app',
    )
    ...
    notify_on_open = forms.BooleanField(
        initial=True,
        required=False,
        help_text='Send notification when email is opened',
    )
    ...
    def clean_to(self):
        return self._parse_emails(self.cleaned_data['to'])

    def clean_cc(self):
        raw = self.cleaned_data.get('cc', '')
        return self._parse_emails(raw) if raw.strip() else []

    def clean_bcc(self):
        raw = self.cleaned_data.get('bcc', '')
        return self._parse_emails(raw) if raw.strip() else []

    def clean_claim_contacts(self):
        raw = self.cleaned_data.get('claim_contacts', '')
        return self._parse_emails(raw) if raw.strip() else []

    def clean(self):
        cleaned = super().clean()

        if not cleaned.get('send_now') and not cleaned.get('scheduled_time'):
            raise ValidationError("Provide a scheduled time or check 'Send now'.")

        if cleaned.get('notify_on_open') and not cleaned.get('admin_notification_email'):
            raise ValidationError(
                "Admin notification email is required when open tracking is enabled."
            )

        # Merge claim contacts into To, deduplicating
        to_list = list(cleaned.get('to') or [])
        claim_contacts = cleaned.get('claim_contacts') or []
        existing = set(to_list)
        for addr in claim_contacts:
