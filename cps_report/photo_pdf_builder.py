...
def _build_room_pdf(rd: dict, styles: dict, page_offset_ref: list) -> bytes:
    ...
    # Sort items by earliest photo so PDF walks through images in order
    if has_attribution and rd['urls']:
        ...
        last_n = first_n + len(items) - 1

    # Build url → [item_nums] map for shared-photo annotation
    ...
    for offset, item in enumerate(items):
        ...
        if has_attribution:
            ...
            if item_urls:
                ...
                for u in item_urls:
                    others = sorted(
                        n for n in url_to_item_nums.get(u, []) if n != item_num
                    )
                    if others:
                        label = (
                            f"↔ item{'s' if len(others) > 1 else ''} "
                ...
                story.extend(
                    _item_image_strip(item_bufs, item_strip_cell_w, shared_notes)
                )
            else:
                story.append(Paragraph(
