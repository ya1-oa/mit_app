"""
Build an Evaluation Report PDF — identical to the Schedule of Loss PDF but
includes each room's AI evaluation notes directly below the room header.
"""
...
C_NOTES_BG   = colors.HexColor('#fffbeb')   # amber tint for AI notes box
C_NOTES_BORDER = colors.HexColor('#fbbf24')
...
def _header_footer(canvas, doc):
    ...
    canvas.drawString(0.6 * inch, 0.35 * inch, 'PPR Evaluation Report  |  All Phase Consulting, LLC')
    canvas.drawRightString(w - 0.6 * inch, 0.35 * inch, f'Page {doc.page}')
    canvas.restoreState()


def build_evaluation_pdf(session) -> bytes:
    buf = BytesIO()
    ...
    room_hdr = ParagraphStyle('RoomHdr', parent=styles['Normal'],
                               ...
                               fontName='Helvetica-Bold')
    notes_lbl = ParagraphStyle('NotesLbl', parent=styles['Normal'],
                                fontSize=8, textColor=colors.HexColor('#92400e'),
                                fontName='Helvetica-Bold', leading=11)
    notes_body = ParagraphStyle('NotesBody', parent=styles['Normal'],
    ...
    # ── Cover page ────────────────────────────────────────────────────────────
    cover_data = [
        [Paragraph('NON SALVAGEABLE / PPR Evaluation Report', h1)],
        [Paragraph('All Phase Consulting, LLC', h2)],
        [Paragraph('Personal Property Replacement — AI Evaluation & Replacement Value Report', h2)],
    ]
    ...
    for rs in room_stats:
        ...
        # ── AI Evaluation Notes (unique to Evaluation Report) ─────────────────
        ai_notes_text = (room.ai_notes or '').strip()
        if ai_notes_text:
            notes_data = [
                [Paragraph('AI Evaluation Notes', notes_lbl)],
                [Paragraph(ai_notes_text, notes_body)],
            ]
            notes_tbl = Table(notes_data, colWidths=[w])
            notes_tbl.setStyle(TableStyle([
                ('BACKGROUND',    (0, 0), (-1, -1), C_NOTES_BG),
                ('TOPPADDING',    (0, 0), (-1, -1), 6),
                ('BOTTOMPADDING', (0, 0), (-1, -1), 6),
                ('LEFTPADDING',   (0, 0), (-1, -1), 10),
                ('RIGHTPADDING',  (0, 0), (-1, -1), 10),
                ('BOX',           (0, 0), (-1, -1), 0.8, C_NOTES_BORDER),
                ('LINEBELOW',     (0, 0), (-1, 0),  0.3, C_NOTES_BORDER),
            ]))
