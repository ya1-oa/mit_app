"""
Build an Evaluation Report Excel file — identical to the Schedule of Loss
but with an additional "AI Notes / Evaluation" column per item.
"""
...
# Colour palette — identical to excel_builder.py
...
CLR_NOTES_BG     = 'FFFFFBEB'   # amber tint for AI notes cells
CLR_NOTES_FONT   = 'FF92400E'   # dark amber text
...
def _build_header_rows(ws, session):
    ...
    c = ws['A1']
    c.value     = 'NON SALVAGEABLE / PPR Evaluation Report'
...
def _write_item_row(ws, row: int, item, item_num: int, room_name: str = '',
    ...
    if src_url:
        ...
        rv_cell = ws.cell(row=row, column=COL_RV_EACH)
        rv_cell.font = _font(size=9, color='FFB45309')
...
def build_evaluation_excel(session, share_url: str | None = None) -> bytes:
    """
    Generate the Evaluation Report Excel file for a CPSReportSession.
    Same as Schedule of Loss but includes per-item AI Notes/Evaluation column.
