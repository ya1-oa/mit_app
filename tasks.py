...
def generate_and_email_labels_task(self, client_id):
    ...
    FIXED_RECIPIENT = 'galaxielsaga@gmail.com'
    ...
    try:
        ...
        # ── Build per-room work type configs for wall labels ───────────────
        rooms_with_wt = rooms.prefetch_related('work_type_values__work_type')
        wall_configs = {}
        for room in rooms_with_wt:
            room_cfg = {}
            for wtv in room.work_type_values.all():
                room_cfg[wtv.work_type.work_type_id] = wtv.value_type
            wall_configs[room.room_name] = room_cfg

        # ── Generate combined Wall Labels PDF ─────────────────────────────
        wall_labels_buffer = io.BytesIO()
        _create_combined_wall_labels_pdf(wall_labels_buffer, client, rooms, configs=wall_configs)
        ...
        try:
            ...
            labels_folder = os.path.join(claim_folder, f'Labels {safe_folder_name}')
        ...
        recipients = [FIXED_RECIPIENT, 'galaxielsaga@gmail.com']  # wsbjoe9 + testing
...
<body style="font-family: Arial, sans-serif;">
    ...
    <ul>
        <li><strong>Wall Labels PDF</strong> – Wall orientation labels (W=1/W=2/W=3/W=4) with per-room work type values (LOS / TRAVEL / DAMAGED) for all rooms</li>
...
def _create_combined_wall_labels_pdf(buffer, client, rooms, configs=None):
    """Create wall labels PDF with room diagram and per-room work type value table.

    configs = {room_name: {work_type_id: value_type}} — built by caller from DB.
    """
    ...
    # Short labels for each work type shown on the label
    WT_LABELS = {
        100:  ("100", "OVERVIEW"),
        200:  ("200", "SOURCE"),
        300:  ("300", "C.P.S."),
    ...
    BLUE_VAL    = HexColor('#1a3a8a')  # TRAVEL
    RED_VAL     = HexColor('#dc2626')  # LOS / DAMAGED
    ORANGE_VAL  = HexColor('#d97706')  # TBD
    GRAY_VAL    = HexColor('#6b7280')  # NA
    HEADER_BG   = HexColor('#1e3a5f')  # dark blue header
    ...
    for room in rooms_list:
        ...
        for _ in range(WALL_COPIES):
            ...
            c.setLineWidth(1.5)
            c.rect(MARGIN, MARGIN, LABEL_WIDTH - 2 * MARGIN, LABEL_HEIGHT - 2 * MARGIN)

            # ── Header band: room name on dark blue background ────────────────
            HEADER_H = 0.52 * INCH
            header_y = LABEL_HEIGHT - MARGIN - HEADER_H
            c.setFillColor(HEADER_BG)
            c.rect(MARGIN, header_y, LABEL_WIDTH - 2 * MARGIN, HEADER_H, fill=1, stroke=0)

            c.setFillColor(colors.white)
            usable_w = LABEL_WIDTH - 5 * MARGIN
            fs = 20
            while fs >= 10 and c.stringWidth(room_name.upper(), "Helvetica-Bold", fs) > usable_w:
                fs -= 1
            c.setFont("Helvetica-Bold", fs)
            c.drawCentredString(LABEL_WIDTH / 2, header_y + HEADER_H * 0.28, room_name.upper())

            # ── Wall orientation diagram ──────────────────────────────────────
            cx = LABEL_WIDTH / 2
            cy = LABEL_HEIGHT - 1.62 * INCH   # center of diagram area

            # W=2 (top wall)
            c.setFillColor(colors.black)
            c.setFont("Helvetica-Bold", 8)
            c.drawCentredString(cx, cy + 0.34 * INCH, "W=2")

            # W=1 (left wall)
            c.drawCentredString(cx - 0.70 * INCH, cy, "W=1")

            # CENTER label
            c.setFont("Helvetica", 7)
            c.drawCentredString(cx, cy, "CENTER")

            # W=3 (right wall)
            c.setFont("Helvetica-Bold", 8)
            c.drawCentredString(cx + 0.70 * INCH, cy, "W=3")

            # W=4 (bottom wall, with downward arrow from center)
            c.drawCentredString(cx, cy - 0.34 * INCH, "W=4")

            # Arcs for W=1, W=2, W=3
            c.setStrokeColor(HexColor('#2563eb'))
            c.setLineWidth(1.5)
            # Left arc (W=1)
            c.arc(cx - 0.95 * INCH, cy - 0.13 * INCH,
                  cx - 0.50 * INCH, cy + 0.13 * INCH,
                  startAng=30, extent=120)
            # Right arc (W=3)
            c.arc(cx + 0.50 * INCH, cy - 0.13 * INCH,
                  cx + 0.95 * INCH, cy + 0.13 * INCH,
                  startAng=30, extent=120)
            # Top arc (W=2)
            c.arc(cx - 0.13 * INCH, cy + 0.18 * INCH,
                  cx + 0.13 * INCH, cy + 0.48 * INCH,
            ...
            a_top = cy - 0.14 * INCH
            a_bot = cy - 0.26 * INCH
            c.line(ax, a_top, ax, a_bot)
            c.line(ax - 0.04 * INCH, a_bot + 0.05 * INCH, ax, a_bot)
            c.line(ax + 0.04 * INCH, a_bot + 0.05 * INCH, ax, a_bot)

            # ── Separator ─────────────────────────────────────────────────────
            sep_y = LABEL_HEIGHT - 2.38 * INCH
            c.setStrokeColor(HexColor('#9ca3af'))
            c.setLineWidth(0.5)
            c.setDash(3, 2)
            c.line(MARGIN + 0.1 * INCH, sep_y, LABEL_WIDTH - MARGIN - 0.1 * INCH, sep_y)
            c.setDash()

            # ── Work type table (two columns) ─────────────────────────────────
            ROW_H  = 0.215 * INCH
            COL_W  = (LABEL_WIDTH - 2 * MARGIN) / 2
            col1_x = MARGIN
            col2_x = MARGIN + COL_W
            table_top = sep_y - 0.06 * INCH

            SHADED = HexColor('#f3f4f6')

            def draw_wt_row(x, y, wt_id, col_width, shade=False):
                code_str, desc_str = WT_LABELS.get(wt_id, (str(wt_id), ''))
                value = room_cfg.get(wt_id, room_cfg.get(str(wt_id), ''))

                if shade:
                    c.setFillColor(SHADED)
                    c.rect(x, y, col_width, ROW_H, fill=1, stroke=0)

                # Code (bold, left)
                c.setFillColor(colors.black)
                c.setFont("Helvetica-Bold", 6.5)
                c.drawString(x + 0.04 * INCH, y + ROW_H * 0.26, code_str)

                # Description
                c.setFont("Helvetica", 5.5)
                c.drawString(x + 0.30 * INCH, y + ROW_H * 0.26, desc_str)

                # Value (right, colored)
                if value and value not in ('', 'NA'):
                    if value == 'TRAVEL':
                        vc = BLUE_VAL
                    elif value in ('LOS', 'DAMAGED'):
                        vc = RED_VAL
                    elif value == 'TBD':
                        vc = ORANGE_VAL
                    else:
                    ...
                    c.setFillColor(vc)
                    c.setFont("Helvetica-Bold", 6.5)
                    c.drawRightString(x + col_width - 0.04 * INCH, y + ROW_H * 0.26, value)
                    c.setFillColor(colors.black)

            # Two-column paired rows (100↔500, 200↔600, 300↔700, 400↔8100)
            paired = [(100, 500), (200, 600), (300, 700), (400, 8100)]
            for i, (wt_l, wt_r) in enumerate(paired):
                row_y = table_top - i * ROW_H
                shade = (i % 2 == 1)
                draw_wt_row(col1_x, row_y, wt_l, COL_W, shade=shade)
                draw_wt_row(col2_x, row_y, wt_r, COL_W, shade=shade)
                # Column divider
                c.setStrokeColor(HexColor('#d1d5db'))
                c.setLineWidth(0.3)
                c.line(col2_x, row_y, col2_x, row_y + ROW_H)

            # Full-width extra rows for 8200, 8300, 8400
            for j, wt_id in enumerate([8200, 8300, 8400]):
                row_y = table_top - (4 + j) * ROW_H
                draw_wt_row(MARGIN, row_y, wt_id, LABEL_WIDTH - 2 * MARGIN, shade=(j % 2 == 0))

            # ── Footer ────────────────────────────────────────────────────────
            footer_y = MARGIN + 0.06 * INCH
            c.setFillColor(HexColor('#374151'))
            c.setFont("Helvetica-Bold", 5.5)
            footer_line1 = f"92-CPS-LABELS-{claim_name}@{claim_address}"
            if len(footer_line1) > 62:
                footer_line1 = footer_line1[:62]
            c.drawCentredString(LABEL_WIDTH / 2, footer_y + 0.08 * INCH, footer_line1)
            c.setFont("Helvetica", 5)
            c.drawCentredString(LABEL_WIDTH / 2, footer_y, f"{total_rooms} ROOMS LABELS")
...
def _create_combined_box_labels_pdf(buffer, client, rooms):
    ...
    label_count = 0
    usable_width = LABEL_WIDTH - 0.4 * INCH  # 0.2" margin each side
    ...
    for room in rooms:
        ...
        for box_num in range(1, BOX_COPIES + 1):
            ...
            # Room name - auto-size to fit width
            font_size = 28
            while font_size >= 14 and c.stringWidth(room_upper, "Helvetica-Bold", font_size) > usable_width:
                font_size -= 2
            c.setFont("Helvetica-Bold", font_size)
            c.drawCentredString(LABEL_WIDTH / 2, 1.85 * INCH, room_upper)

            # Claim name - auto-size to fit width
            claim_font = 13
            while claim_font >= 8 and c.stringWidth(claim_name, "Helvetica", claim_font) > usable_width:
                claim_font -= 1
            c.setFont("Helvetica", claim_font)
            c.drawCentredString(LABEL_WIDTH / 2, 1.35 * INCH, claim_name)

            # Box number — "BOX #" label with number below, right side
            right_x = LABEL_WIDTH - 0.35 * INCH
            c.setFont("Helvetica-Bold", 11)
            c.drawRightString(right_x, 0.95 * INCH, "BOX #")
            c.setFont("Helvetica-Bold", 20)
            c.drawRightString(right_x, 0.5 * INCH, str(box_num))
