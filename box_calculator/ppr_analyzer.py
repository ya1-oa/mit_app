"""
PPR (Pre-Packout Report) AI box count estimator.

Uses Claude Vision to analyze room photos and return direct box count estimates
using the 11 PPR column types used by pack-out crews, based on standard
...
def analyze_room_ppr(
    ...
    try:
        ...
        counts = {col: max(0, int(parsed.get(col, 0) or 0)) for col in PPR_COLUMNS}
...
def _error_result(error: str, images_used: int) -> dict:
    return {
        "success": False,
        "counts": {col: 0 for col in PPR_COLUMNS},
        "total": 0,
        "confidence": "none",
        "notes": "",
        "images_used": images_used,
